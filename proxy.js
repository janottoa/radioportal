...
const planetradioPollers = new Map();
let lastProcessedMetadata = null; // For å unngå loop ved like metadata
...
// Oppdater metadata fra alle aktive script-strømmer
...
// Behandle stream URL og bestem metadata-kilde
                ...
                for (const category in stationsData) {
                    if (Array.isArray(stationsData[category])) {
                        const station = stationsData[category].find(s =>
                            s.url === streamUrl || (streamUrl && streamUrl.includes(s.url))
                        );

                        if (station && station.app) {
...
function stopStream(streamId) {
    if (activeScriptStreams.has(streamId)) {
        ...
        if (activeScriptStreams.size === 0 && scriptCheckInterval) {
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', async (message) => {
        ...
        try {
            ...
            logError(`Feil ved behandling av strøm: ${error.message}`);
        }
    });
...
async function fetchAlbumCover(artist, track) {
    try {
        ...
        // Prøv først med eksakt søk
        let response = await axios.get(
            `https://api.deezer.com/search?q=artist:"${encodeURIComponent(artist)}" track:"${encodeURIComponent(track)}"&limit=1`
        );

        logInfo(`Deezer response status: ${response.status}, antall treff: ${response.data.data?.length || 0}`);

        // Hvis ingen treff, prøv enklere søk
        if (!response.data.data || response.data.data.length === 0) {
            logInfo(`Prøver alternativt søk for: ${artist} - ${track}`);
            response = await axios.get(
                `https://api.deezer.com/search?q=${encodeURIComponent(artist + ' ' + track)}&limit=5`
            );
            
            // Finn beste match blant resultatene
            if (response.data.data && response.data.data.length > 0) {
                const bestMatch = response.data.data.find(t => 
                    t.title.toLowerCase().includes(track.toLowerCase()) ||
                    track.toLowerCase().includes(t.title.toLowerCase())
                ) || response.data.data[0];
                
                response.data.data = [bestMatch];
                logInfo(`Fant alternativ match: "${bestMatch.title}" av ${bestMatch.artist.name}`);
            }
        }
        ...
        if (response.data.data && response.data.data.length > 0) {
            ...
            logInfo(`Deezer track funnet: "${deezerTrack.title}" av ${deezerTrack.artist.name}`);
            logInfo(`Album info: ${JSON.stringify(deezerTrack.album)}`);

            // Fallback til mindre cover-størrelser hvis cover_xl mangler
            const albumCover = deezerTrack.album?.cover_xl || deezerTrack.album?.cover_big || deezerTrack.album?.cover_medium || deezerTrack.album?.cover_small || null;
            
            if (!albumCover) {
                logInfo(`ADVARSEL: Ingen cover-bilder funnet for album: ${deezerTrack.album?.title}`);
            }
        ...
        logInfo(`Ingen treff fra Deezer for: ${artist} - ${track}`);
        return null;
    } catch (error) {
        logError(`Feil ved henting fra Deezer: ${error.message}`);
        if (error.response) {
            logError(`Deezer API feil: status ${error.response.status}, data: ${JSON.stringify(error.response.data)}`);
        }
        return null;
    }
}
...
async function fetchLyrics(artist, title) {
    let cleanArtist = artist.split(/[,/]|feat\.|featuring|&|ft\.| and /i)[0].trim();
    let cleanTitle = title.split(/[\(\[]/)[0].trim();

    logInfo(`Prøver LRCLIB for: [${cleanArtist}] - [${cleanTitle}]`);

    // Prøv først med artist + title
    let lyricsData = await tryFetchLyrics(cleanArtist, cleanTitle);
    
    // Hvis ikke funnet, prøv omvendt: title + artist
    if (!lyricsData.hasLyrics) {
        logInfo(`Ingen lyrics for ${cleanArtist} - ${cleanTitle}, prøver omvendt...`);
        lyricsData = await tryFetchLyrics(cleanTitle, cleanArtist);
    }
...
async function tryFetchLyrics(artist, title) {
    logInfo(`LRCLIB søk: [${artist}] - [${title}]`);
    ...
    try {
        ...
        if (!response || !response.data || (!response.data.syncedLyrics && !response.data.plainLyrics)) {
            logInfo(`Ingen direkte match på LRCLIB. Prøver søk...`);
            ...
            if (searchResponse.data && searchResponse.data.length > 0) {
                const bestMatch = searchResponse.data.find(res => res.syncedLyrics || res.plainLyrics) || searchResponse.data[0];
        ...
        if (response && response.data && (response.data.syncedLyrics || response.data.plainLyrics)) {
            ...
            return {
                ...
                lyricsLines: d.plainLyrics ? d.plainLyrics.split('\n').map(l => l.trim()).filter(l => l) : [],
                source: 'lrclib'
            };
        }
    } catch (error) {
        logInfo(`LRCLIB-feil, prøver backup...`);
    }

    // Prøv OVH som backup
    try {
        const ovhResponse = await axios.get(
        ...
        if (ovhResponse.data && ovhResponse.data.lyrics) {
            return {
                hasLyrics: true,
                syncedLyrics: null,
                lyricsLines: ovhResponse.data.lyrics.split('\n').map(l => l.trim()).filter(l => l),
                source: 'ovh'
            };
        }
    } catch (e) {
    ...
    return { hasLyrics: false, lyricsLines: [], syncedLyrics: null };
}
...
async function startFffmpegProcess(result, ws) {
    ...
    function spawnFffmpeg() {
        ...
        // Send initial metadata etter 1 sekund
        if (result.initialMetadata && currentStreamId) {
            setTimeout(() => {
                if (currentFffmpegProcess) {
                    const streamTitle = `${result.initialMetadata.artist} - ${result.initialMetadata.title}`;
                    currentFffmpegProcess.stderr.emit('data', Buffer.from(`Metadata update for StreamTitle: ${streamTitle}`));
                }
            }, 1000);
        }
        ...
        ffmpeg.stderr.on('data', async (data) => {
            ...
            // Hvis Planetradio, ignorer FFmpeg-metadata
            ...
            if (titleMatch) {
                let streamTitle = fixEncoding(titleMatch[1].trim());
                streamTitle = streamTitle.split('||')[0].trim();

                // Sjekk om metadata er lik forrige - unngå loop
                if (lastProcessedMetadata === streamTitle) {
                    logInfo(`Metadata uendret, hopper over: ${streamTitle}`);
                    return;
                }
                lastProcessedMetadata = streamTitle;
                ...
                if (trackInfoMatch) {
                    title = fixEncoding(trackInfoMatch[1].trim());
                    artist = fixEncoding(trackInfoMatch[2].trim());
                } else {
                    title = fixEncoding(processedMetadata.title || streamTitle);
                    artist = fixEncoding(processedMetadata.artist || null);
                }

                let deezerData = null;
                if (artist && title && !title.includes('Metadata update for StreamUrl')) {
                    deezerData = await fetchAlbumCover(artist, title);
                    logInfo(`Deezer data mottatt: ${deezerData ? 'JA' : 'NULL'}, cover: ${deezerData?.album_cover ? 'JA' : 'NEI'}`);
                }

                const lyricsData = artist && title
                    ? await fetchLyrics(artist, title)
                    : { hasLyrics: false, lyricsLines: [], syncedLyrics: null };
...
function stopFffmpegProcess() {
    if (currentFffmpegProcess) {
        ...
        lastProcessedMetadata = null; // Nullstill for ny strøm
    ...
    try {
        ...
        if (data && data.TrackId) {
            ...
            const metadata = {
                ...
                explicit: deezerData.explicit_lyrics,
                preview: deezerData.preview,
                imageUrl: deezerData.album_cover,
                deezer: deezerData,
                lyrics: {
                    hasLyrics: lyricsData.hasLyrics,
                    lyricsLines: lyricsData.lyricsLines || [],
        ...
        logError(`Planetradio metadata fetch error: ${error.message}`);
    }
}

async function startPlanetradioPolling(streamUrl, channelUrl, ws) {
    logInfo(`Starter Planetradio polling: ${channelUrl}`);
    ...
    const pollInterval = setInterval(async () => {
        try {
            ...
            if (data && data.TrackId && data.TrackId !== lastTrackId) {
...
app.post('/save-song', async (req, res) => {
    ...
    try {
        ...
        const filePath = path.join(savedSongsDir, 'saved_songs.txt');
        ...
        res.json({ success: true, message: `Lagret: ${artist} - ${title}`, savedAt: dateTime });
    } catch (error) {
    ...
    try {
        ...
        const songs = content.split('\n')
            ...
            .map(line => {
                ...
                return { artist: parts[0] || '', title: parts[1] || '', savedAt: parts[2] || '' };
            });
...
// === START SERVER ===
app.listen(API_PORT, () => {
    logInfo(`Metadata API kjører på port ${API_PORT}`);
    logInfo(`WebSocket server på port ${WEBSOCKET_PORT}`);
    logInfo(`Metadata WebSocket på port ${METADATA_PORT}`);
});