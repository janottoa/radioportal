...
wss.on('connection', (ws) => {
    ws.on('message', (streamUrl) => {
        if (streamUrl === currentStreamUrl) {
            logError(`Stream URL er allerede valgt: ${streamUrl}`);
            return;
        }
...
function startFfmpegProcess(streamUrl, ws) {
    stopFfmpegProcess(); // Avslutt eventuelle kjørende prosesser først
    ...
    function spawnFfmpeg() {
        ffmpeg = spawn('ffmpeg', [
            '-i', streamUrl,
            '-af', 'loudnorm=i=-16:tp=-1.5:lra=11', // Normaliseringsfilter for jevnt volum
            '-vn',
            '-loglevel', 'debug',
            '-f', 'null', '-'
        ]);
