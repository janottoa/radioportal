...
// Funksjon for å håndtere metadata og hente albumcover fra Spotify om nødvendig
async function handleMetadata(metadata) {
    if (!metadata.imageUrl) {
        console.log("Ingen gyldig imageUrl i metadata, henter fra Spotify...");
        const spotifyImageUrl = await fetchSpotifyAlbumCover(metadata.artist, metadata.title);
        if (spotifyImageUrl) {
            metadata.imageUrl = spotifyImageUrl; // Oppdaterer metadata med Spotify-albumcoveret
        }
    }
    return metadata;
}
    ...
    proxySocket.on('message', async (data) => {
        ...
        if (parsedMessage.StreamTitle) {
            ...
            try {
                ...
                // Håndter metadata for å hente albumcover om nødvendig
                const updatedMetadata = await handleMetadata(messageToFrontend);
