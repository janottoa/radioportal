    ...
    if access_token:
        ...
        if album_cover_url:
            return jsonify({
                ...
                'artist': artist,
                'track': track,
                'album_name': album['name'],  # Navn på album
                'track_spotify_id': track_data['id'],  # Spotify-ID for sangen
                'popularity': track_data['popularity'],  # Popularitet
                'artists': [{'name': artist['name'], 'spotify_id': artist['id']} for artist in track_data['artists']],  # Alle artister
                'spotify_uri': track_data['uri'],  # Spotify URI
                'external_url': track_data['external_urls']['spotify']  # Spotify lenke
            })
        else:
