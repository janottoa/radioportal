...
const nrkFix = require('./NRKFix.js');
    ...
    function spawnFfmpeg() {
        ...
        ffmpeg.stderr.on('data', async (data) => {
            ...
            if (titleMatch) {
                ...
                // Split StreamTitle for å få artist og sangnavn
                ...
                // Hvis imageUrl mangler, hent albumcover fra Spotify
                if (!metadata.imageUrl && metadata.artist && metadata.title) {
                    console.log("Ingen imageUrl funnet, henter fra Spotify...");
                    try {
                        const spotifyImageUrl = await fetchSpotifyAlbumCover(metadata.artist, metadata.title);
                        if (spotifyImageUrl) {
                            metadata.imageUrl = spotifyImageUrl;
                            console.log(`Hentet bilde fra Spotify: ${spotifyImageUrl}`);
                        }
                    } catch (error) {
                        console.error(`Feil ved henting av Spotify-albumcover: ${error.message}`);
                    }
                }

                // Send metadata til frontend og metadataFetcherClient hvis tilgjengelig
                ...
                logError(`Oppdatert StreamTitle: ${streamTitle}, imageUrl: ${metadata.imageUrl || 'Ingen bilde tilgjengelig'}`);
            }
